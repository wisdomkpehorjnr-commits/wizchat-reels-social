...
const ThemeSelector = () => {
  ...
  const themes = [
    { value: 'light' as const, label: 'Light', desc: 'Clean & bright', colors: ['#ffffff', '#16a34a', '#f0fdf4'], free: true },
    { value: 'dark' as const, label: 'Dark', desc: 'Easy on the eyes', colors: ['#0f172a', '#16a34a', '#1e293b'], free: true },
    { value: 'ultra' as const, label: 'Ultra', desc: 'Pure black & white', colors: ['#000000', '#ffffff', '#1a1a1a'], free: false },
    { value: 'ghana' as const, label: 'Ghana Pride', desc: 'Red, Gold & Green', colors: ['#ce1126', '#fcd116', '#006b3f'], free: false },
  ];

  const handleThemeClick = (theme: typeof themes[0]) => {
    if (!theme.free && !purchasedThemes.includes(theme.value)) {
      toast({
        title: "🔒 Theme Locked",
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      {themes.map((t) => {
        const isLocked = !t.free && !purchasedThemes.includes(t.value);
        return (
          <button
            key={t.value}
