...
// =============================================
const STORIES_CACHE_KEY = 'wizchat_stories_cache';
const STORIES_MIN_REFRESH_INTERVAL_MS = 2 * 60 * 1000; // 2 minutes
...
const initializeStoriesFromLocalStorage = () => {
  if (storiesStore.hasInitialized) return;
  ...
  try {
    ...
    if (cached) {
      ...
      if (parsed?.data && Array.isArray(parsed.data)) {
        ...
        storiesStore.stories = parsed.data.map((s: any) => ({
          ...s,
        ...
        storiesStore.lastFetchTime = parsed.timestamp || 0;
        console.debug('[Stories] Restored from localStorage:', storiesStore.stories.length);
      }
    }
  } catch (e) {
    console.debug('[Stories] localStorage parse failed:', e);
  }
  storiesStore.hasInitialized = true;
};
...
const StoriesSection: React.FC = () => {
  ...
  const [stories, setStories] = useState<Story[]>(storiesStore.stories);
  const [groupedStories, setGroupedStories] = useState<GroupedStory[]>([]);
  const [loading, setLoading] = useState(storiesStore.stories.length === 0);
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    const timeSinceLastFetch = now - storiesStore.lastFetchTime;
    const isCacheStale = timeSinceLastFetch > STORIES_MIN_REFRESH_INTERVAL_MS;

    if (storiesStore.stories.length > 0 && !isCacheStale) {
      console.debug('[Stories] Using cached stories, skipping fetch');
      setLoading(false);
      return;
    }

    loadStories(storiesStore.stories.length > 0);
  ...
  // Sync stories to store when they change
  useEffect(() => {
    if (stories.length > 0) {
  ...
  // Show cached stories instantly, no skeleton if we have data
        ...
        <DialogContent className="max-w-md backdrop-blur-md bg-card border-green-500/30">
          ...
          <div className="space-y-4">
            {storyPreview && (
              <div className="relative">
                {mediaType === 'image' ? (
                  <img src={storyPreview} alt="Story preview" className="w-full h-60 object-cover rounded-lg" />
                ) : (
                  <video src={storyPreview} controls className="w-full h-60 rounded-lg" />
                )}
